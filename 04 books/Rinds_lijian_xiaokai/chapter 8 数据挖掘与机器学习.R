...
setwd("F:\\百度云同步盘\\Data Science\\01 R\\03 分析挖掘\\cluster")

# 8.2.1 层次聚类
## 层次聚类也成为系统聚类，先把每个样本都单独作为一类，然后根据距离进行合并，直到所有的样本都合并为一类；
...
## 层次聚类的优缺点：
##（1）优点：基于距离矩阵进行聚类，不需要原始数据；可用于不同形状的聚类；聚类前无需确定聚类个数
...
## 聚类的关键是距离计算方法的选择，主要有：欧式距离\曼哈顿记录\两项距离\闵可夫斯基距离\相关系数\夹角余弦\马氏距离……
...
## K均值聚类又称为动态聚类，需要先确定聚类的个数，主要参数有：数据对象\聚类个数\取随机初始中心的次数(默认为1)
clusteModel <- kmeans(dataScale, centers = 3, nstart = 10)
class(clusteModel)
table(iris$Species, clusteModel$cluster)

## kmeans函数默认采用欧氏距离，如果要使用其他距离计算方法，则使用cluster包的pam函数，配合proxy包来计算
require("proxy")
require("cluster")
...
## K均值聚类最初的类中心是随机选择的，这样可能会形成较差的聚类结果，如何改进使效果更好呢？方法有很多：
...
# 8.2.3 基于密度的聚类
## K均值聚类不适用于非球形的聚类，对于非球形聚类，我们需要用基于密度的聚类，常用的是DBSCAN方法
